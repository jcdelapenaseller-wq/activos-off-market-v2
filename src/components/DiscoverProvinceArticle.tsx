        ...
        <article className="bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-slate-200">
          ...
          <div className="prose prose-lg prose-slate max-w-none">
            ...
            <div className="my-12 flex flex-col sm:flex-row gap-4 w-full border-t border-slate-200 pt-10">
              <Link 
                to={topExamples[0] ? `/subasta/${topExamples[0].slug}` : `/subastas/${provinceName.toLowerCase().replace(/\s+/g, '-')}`} className="flex-1 inline-flex items-center justify-center gap-2 bg-brand-600 text-white font-bold px-8 py-4 rounded-xl hover:bg-brand-700 transition-colors shadow-sm text-center"
              >
                Ver análisis técnico <ArrowRight size={18} />
              </Link>
              <Link 
                to={`/subastas/${provinceName.toLowerCase().replace(/\s+/g, '-')}`} className="flex-1 inline-flex items-center justify-center gap-2 bg-slate-100 text-slate-700 font-bold px-8 py-4 rounded-xl hover:bg-slate-200 transition-colors text-center"
              >
